...
tableTransform =function(data_df, cols_to_remove=NULL){
  ...
  return(dataDF %>% select(c(Name, Age, year,  HomeTown, HomeState, HomeCountry, RawTime_S, RawTime_M, RawPace_S, RawPace_M, sex, section, PiS, TiS, PiD, TiD)))
}
```
...
```{r}
columns = c("Name", "Age", "year",  "HomeTown", "HomeState", "HomeCountry", "RawTime_S", "RawTime_M", "RawPace_S", "RawPace_M", "sex", "section", "PiS", "TiS", "PiD", "TiD")
...
## We will remove NAs and only focus on completed records and rewmove NAs for further processing 
```{r}
