...
#next, we calculate the number of minutes into a new colume called TimeMinutes
df$TimeMinutes = (df$Hours * 60) + df$Minutes + (df$Seconds/60)
head(df)
```

Saving the dataframe for later retrieval
```{r}
#write.csv(df,"C:/Users/blgai/OneDrive/Documents/School/SMU/Spring 2021/Quantifying the World/Case4/files/women_data.csv", row.names = FALSE)
```

load the data if necessary
```{r}
file_path = "C:/Users/blgai/OneDrive/Documents/School/SMU/Spring 2021/Quantifying the World/Case4/files/women_data.csv"
...
```
let's create a new column to hold the pace per mile
```{r}
df$PaceMinutes = (df$TimeMinutes/10)
df$PaceBin = cut(df$PaceMinutes, breaks=c(0,5,5.5,6,6.5,7,7.5,8,8.5,9,9.5,10,10.5,11,18), 
                 labels=c("1-5","5.1-5.5","5.5-6","6-6.5","6.5-7","7-7.5","7.5-8","8-8.5","8.5-9","9-9.5","9.5-10","10-10.5","10.5-11","11-18"))
```
...
mu <- ddply(df, "year", summarise, grp.mean=mean(Age))
...
```
Let's look at the volume of runners by pace bin
```{r}

ggplot(df %>% group_by(year, PaceBin) %>% tally()) +
  geom_col(mapping = aes(x=PaceBin, y=n)) +
  facet_grid(vars(year)) +
  labs(title="Distribution of pace throughout the years")
  
```
Let's examine the volume of runners by year by age group to see if there has been an increase in the number of younger runners
```{r}
ggplot(df %>% group_by(year, AgeBin) %>% tally()) +
...
Let's get a sense of the distribution of age over the years
```{r}
f = ggplot(df %>% group_by(year, AgeBin) %>% tally())
f + geom_col(mapping = aes(x=AgeBin, y=n)) + facet_wrap(vars(year)) + theme_classic() + labs(title="Boxplot of Age Group by Year")
```


```{r}
ggplot(df %>% group_by(year) %>% tally()) +
...
```{r}
dfRunVol = df %>% group_by(year) %>% tally()
...
```{r}
summary(df$PaceMinutes)
```


```{r}
ggplot(df) +
  geom_boxplot(mapping = aes(x=AgeBin))
```
