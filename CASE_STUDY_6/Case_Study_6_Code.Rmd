...
#grid search indicates max_depth 7 is best, but appears to be slightly overfit.  let's try with 6
...
```{r}
#make the prediction
pred_train = predict(xgbFinal, data.matrix(train[,-1]))
pred_train_label = as.numeric(pred_train>.88)
confusionMatrix(factor(pred_train_label), factor(train$isSpam) , positive = "1")
```
