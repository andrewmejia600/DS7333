...
#Then we find the error in our estimates
#Page 38
```{r}
...
actualFold = onlineFold[ , c("posX", "posY")]
for (k in 1:K) {
  estFold = predXY(newSignals = onlineFold[ , 6:11], newAngles = onlineFold[ , 4], offlineFold, numAngles = numAngles, k = k)
  err[k] = err[k] + calcError(estFold, actualFold)
}
plot(err, type="b", xlab="k-value", ylab="Error")
```
#PG 39
```{r}
K=20
...
#PG 39
```{r}
#WARNING:  this code chunk takes a prohibitively long time to run...  suggest breaking down into smaller separate chunks
K=20
err = rep(0,K)
...
```{r}
#I need to be able to iterate through the MAC scenarios and generate the elbow plot while also checking for the best number of angles
K=20
numAngles = 3
...
for (f in 1:length(macPerms)){
  ...
  #subset the online and offline data to keep only the Mac addresses of interest
  ...
  for (j in 1:v){
    ...
    actualFold = onlineFold[ , c("posX", "posY")]
    for (a in 1:numAngles){
      for (K in 1:K){
        estFold = predXY(newSignals = onlineFold[ , 6:11], newAngles = onlineFold[ , 4], offlineFold, numAngles = a, k = K)
        myError = calcError(estFold, actualFold)
        #the following checks if this is the first instance we're loading the results dataframe
        if ((f==1)&(K == 1)){
          dfResults <- data.frame("Scenario"=macPerms[f],"AngleCount"=a,"K_value"=K,"Error"=myError)
        }else{
          dfRow <- data.frame("Scenario"=macPerms[f],"AngleCount"=a,"K_value"=K,"Error"=myError)
...
geom_line(mapping = aes(x=K_value, y=Error_Avg, color=AngleCount), size=1.2) + facet_grid(rows=vars(AngleCount), cols=vars(Scenario)) + theme_classic() + title="Elbow Plot by Mac Scenario and Number of Angles"
```
...
```{r}
dfResults <- data.frame("Scenario"=factor(),"AngleCount"=integer(),"K_value"=integer(),"Error"=double())
dfResults <- data.frame("Scenario"="Drop: 00:0f:a3:39:e1:c0","AngleCount"=1,"K_value"=1,"Error"=22.2)
dfRow<- data.frame("Scenario"="Drop: 00:0f:a3:39:dd:cd","AngleCount"=2,"K_value"=2,"Error"=33.3)
dfResults <- rbind(dfResults,dfRow)
dfResults
```
