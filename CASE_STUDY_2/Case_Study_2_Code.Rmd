...
```{r}
offline_f_path = "/media/andrew/Seagate Backup Plus Drive/Documents/School/HomeWork/QTW/DS7333/Data/offline.final.trace.txt"
online_f_path = "/media/andrew/Seagate Backup Plus Drive/Documents/School/HomeWork/QTW/DS7333/Data/online.final.trace.txt"
```
...
```{r}
# Unlike text example, we will initialize sampleAngle as false, so we do not select a sample of angles, if True, the ans x will take a randon sample from the 45 degree angles. This sampling technique was borrowed from 
#https://github.com/ngupta23/ds7333_qtw/tree/master/case_study_1/submission_Kannan_Moro_Gupta/code
...
predXY = function(newSignals, newAngles, trainData, numAngles = 1, k = 3, weighted=FALSE){
  ...
  k_means_single_obs = function(df, k){
    weights = df[1:k, 5]
    weighted_x = sum(df[1:k, 2] * weights) / sum(weights)
    weighted_y = sum(df[1:k, 3] * weights) / sum(weights)
